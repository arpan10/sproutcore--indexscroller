...
/** @scope IndexScroller.IndexScrollerView.prototype */ {
	...
	*/
	...
	mouseDown: function(evt) {
		var target = Event.element(evt); alert(el);
//		var target = el.targetElement ; alert(target);
		var childNodes = this.rootElement.childNodes;
		var idx = childNodes.length ;
		var ret = null ;
		while(ret === null && (--idx >= 0)) {
			var el = childNodes[idx] ;
			if (el === target) ret = idx ;
		}
		alert(ret);
		this.set('value', ret);
	...
	showIndexes: function() {
		...
		for (var i = 0; i < this.groups.get('length'); i++)
		{
			...
			buffer += "<li class=\"" + sel_class + "\">" + this.groups.objectAt(i).name + "</li>";
		}
		buffer += "</ul>";
		buffer += "</div>";
