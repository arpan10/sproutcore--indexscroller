...
/** @scope IndexScroller.IndexScrollerView.prototype */ {
	...
	*/
	...
	mouseDragged: function(evt) {
		var el = Event.element(evt);
		this.changeIndex(parseInt(el.id));
	},
	
	changeIndex: function(new_idx) {
	...
	showIndexes: function() {
		var buffer = "";
		var sel_class;
		buffer += "<ul class=\"index-scroller-view\">"
		for (var i = 0; i < this.groups.get('length'); i++) {
			if (this.value === this.groups.objectAt(i).value)
				sel_class = "sel";
			else sel_class = "";
			
			buffer += "<li id=\"" + this.groups.objectAt(i).value + "\" class=\"index-group " + sel_class + "\">" + this.groups.objectAt(i).name + "</li>";
		}
		buffer += "</ul>";
